...
public class LunchRoom
{
   ...
   { 
      ...
      *************************/
      PriorityQueue<Customer> lunchLine = new PriorityQueue<Customer>();
      int wait = 0;
      int tempGradeLevel = 0;
      
      int numOfSeniors = 0;
      int numOfJuniors = 0;
      int numOfSophomores = 0;
      int numOfFreshmen = 0;
      
      int longestSenior = 0;
      int longestJunior = 0;
      int longestSophomore = 0;
      int longestFreshman = 0;
      
      int totalSeniorWait = 0;
      int totalJuniorWait = 0;
      int totalSophomoreWait = 0;
      int totalFreshmanWait = 0;
      
      for (int i = 1; i <= TIME; i++) {
         if (Math.random() < CHANCE_OF_CUSTOMER) {
            double gradeLevel = Math.random();
            if (gradeLevel <= .25) {
               tempGradeLevel = 1;
               numOfFreshmen++; 
            }  else if (gradeLevel > .25 && gradeLevel <= .5) {
               tempGradeLevel = 2;
               numOfSophomores++;
            } else if (gradeLevel > .5 && gradeLevel <= .75) {
               tempGradeLevel = 3;
               numOfJuniors++;
            } else if (gradeLevel > .75 && gradeLevel <= 1) {
               tempGradeLevel = 4;
               numOfSeniors++;
            }
            
            if (lunchLine.peek() != null && tempGradeLevel > lunchLine.peek().gradeLevel1) {
               wait = 0;
            }
            
            lunchLine.add(new Customer(gradeLevel, i));
         }
         displayTimeAndQueue(i, lunchLine);
         if (lunchLine.peek() != null) {
            if (wait == lunchLine.peek().wait) {
               if (lunchLine.peek().gradeLevel1 == 4 && i - lunchLine.peek().minuteArrived1 > longestSenior) {
                  longestSenior = i - lunchLine.peek().minuteArrived1;
               }
               if (lunchLine.peek().gradeLevel1 == 3 && i - lunchLine.peek().minuteArrived1 > longestJunior) {
                  longestJunior = i - lunchLine.peek().minuteArrived1;
               }
               if (lunchLine.peek().gradeLevel1 == 2 && i - lunchLine.peek().minuteArrived1 > longestSophomore) {
                  longestSophomore = i - lunchLine.peek().minuteArrived1;
               }
               if (lunchLine.peek().gradeLevel1 == 1 && i - lunchLine.peek().minuteArrived1 > longestFreshman) {
                  longestFreshman = i - lunchLine.peek().minuteArrived1;
               }
               
               if (lunchLine.peek().gradeLevel1 == 4) {
                  totalSeniorWait += i - lunchLine.peek().minuteArrived1;
               } else if (lunchLine.peek().gradeLevel1 == 3) {
                  totalJuniorWait += i - lunchLine.peek().minuteArrived1;
               } else if (lunchLine.peek().gradeLevel1 == 2) {
                  totalSophomoreWait += i - lunchLine.peek().minuteArrived1;
               } else {
                  totalFreshmanWait += i - lunchLine.peek().minuteArrived1;
               }
         ...
         if (lunchLine.peek() != null) {
            outfile.println(lunchLine.peek() + " is being served for " + (lunchLine.peek().wait - wait) + " minutes.");
         }
      }
      
      int countmins = 1801;
      
      while (lunchLine.peek() != null) {
         displayTimeAndQueue(countmins, lunchLine);
         if (lunchLine.peek() != null) {
            if (wait == lunchLine.peek().wait) {
               if (lunchLine.peek().gradeLevel1 == 4 && countmins - lunchLine.peek().minuteArrived1 > longestSenior) {
                  longestSenior = countmins - lunchLine.peek().minuteArrived1;
               }
               if (lunchLine.peek().gradeLevel1 == 3 && countmins - lunchLine.peek().minuteArrived1 > longestJunior) {
                  longestJunior = countmins - lunchLine.peek().minuteArrived1;
               }
               if (lunchLine.peek().gradeLevel1 == 2 && countmins - lunchLine.peek().minuteArrived1 > longestSophomore) {
                  longestSophomore = countmins - lunchLine.peek().minuteArrived1;
               }
               if (lunchLine.peek().gradeLevel1 == 1 && countmins - lunchLine.peek().minuteArrived1 > longestFreshman) {
                  longestFreshman = countmins - lunchLine.peek().minuteArrived1;
               }
               
               if (lunchLine.peek().gradeLevel1 == 4) {
                  totalSeniorWait += countmins - lunchLine.peek().minuteArrived1;
               } else if (lunchLine.peek().gradeLevel1 == 3) {
                  totalJuniorWait += countmins - lunchLine.peek().minuteArrived1;
               } else if (lunchLine.peek().gradeLevel1 == 2) {
                  totalSophomoreWait += countmins - lunchLine.peek().minuteArrived1;
               } else {
                  totalFreshmanWait += countmins - lunchLine.peek().minuteArrived1;
               }
      ...
      System.out.println("Senior\t\t\t" + numOfSeniors + "\t\t\t" + longestSenior + "\t\t\t\t\t\t" + (totalSeniorWait / numOfSeniors));
      System.out.println("Junior\t\t\t" + numOfJuniors + "\t\t\t" + longestJunior + "\t\t\t\t\t\t" + (totalJuniorWait / numOfJuniors));
      System.out.println("Sophomore\t\t" + numOfSophomores + "\t\t\t" + longestSophomore + "\t\t\t\t\t" + (totalSophomoreWait / numOfSophomores));
      System.out.println("Freshman\t\t\t" + numOfFreshmen + "\t\t\t" + longestFreshman + "\t\t\t\t\t" + (totalFreshmanWait / numOfFreshmen));
     }
   ...
   public static class Customer implements Comparable<Customer>
   {
      ...
      public Customer (double gradeLevel, int minuteArrived) {
         if (gradeLevel <= .25) {
            grade = "Fr";
            gradeLevel1 = 1;
         }  else if (gradeLevel > .25 && gradeLevel <= .5) {
         ...
         minuteArrived1 = minuteArrived;
      }
      
      public String toString() {
         return String.valueOf(grade) + ":" + minuteArrived1;
      }
      ...
      public int compareTo(Customer obj) {
         if (obj.gradeLevel() < this.gradeLevel()) {
            return -1;
         } else if (obj.gradeLevel() > this.gradeLevel()) {
            return 1;
         } else {
            return 0;
         }
